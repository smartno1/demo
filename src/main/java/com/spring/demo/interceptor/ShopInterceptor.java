...
public class ShopInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        log.info("shop interceptor preHandle() Start");

        if (!isLogin(session)) {
            log.info("this request deny!");

            response.sendRedirect("/member/sign-in?message=no-login");
            return false;
        }
        log.info("shop interceptor preHandle() END");
        return true;
    }
...
//
//        log.info("shop interceptor postHandle() Start");
//        // postHandle이 작동해야 하는 URI 목록
//        List<String> uriList = Arrays.asList("/shop/modify", "/shop/delete","/shop/regist");
//
//        // 현재 요청 URI 정보 알아내기
...
//            log.info("shop interceptor postHandle() - match modify,delete,GET! ");
//
//            HttpSession session = request.getSession();
//
